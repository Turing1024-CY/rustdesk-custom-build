...
def ffi_bindgen_function_refactor():
    # workaround ffigen
    bridge_file = 'flutter/lib/generated_bridge.dart'
    if os.path.exists(bridge_file):
        system2(
            'sed -i "s/ffi.NativeFunction<ffi.Bool Function(DartPort/ffi.NativeFunction<ffi.Uint8 Function(DartPort/g" %s' % bridge_file)
    else:
        print('Warning: %s not found, skipping sed replacement' % bridge_file)
...
def build_flutter_deb(version, features):
    ...
    # Copy pre-configured RustDesk config files
    system2('mkdir -p tmpdeb/etc/rustdesk/config/')
    system2('cp ../res/RustDesk.toml tmpdeb/etc/rustdesk/config/')
    system2('cp ../res/RustDesk2.toml tmpdeb/etc/rustdesk/config/')
